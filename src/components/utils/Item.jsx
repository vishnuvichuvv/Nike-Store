...
const dispatch= useDispatch()
    ...
    const onAddToCart = () =>{
        const item={
            id, title, text, img, color, shadow, price}
  ...
  return (
    ...
    <div className={`relative bg-gradient-to-b ${color} ${shadow}
         grid items-center ${ifExists ? 'justify-items-start' : 'justify-items-center'}  rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
        <div className={`grid items-center ${ifExists ? 'justify-items-start' : 'justify-items-center'}`}>
            ...
            <div className='flex items-center -bejustifytween w-28 my-2'>
            ...
            <div className='flex items-center gap-3'>
                ...
                button-theme px-2 py-1 shadow shadow-sky-200 text-sm text-black' onClick={()=>{onAddToCart(); onCartToggle();}}>{btn} </button>
            </div>
        </div>
